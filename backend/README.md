...
Run `python -m venv myenv` to create the `mynev` virtual environment. Activate it by running `source myenv/bin/activate` (for macOS users) or `myenv\Scripts\activate` (for Windows users).
