...
import { getMeals, type Meal } from "../../../api/apiClient";
...
export function MealPicker(): JSX.Element {
  ...
  return (
    <div>
      <button onClick={() => setFormattedDate(formatDate(changeDate(day, false)))}>{"<"}</button>
      <h2>{formattedDate}</h2>
      <button
      ...
      <MealDetail
        meals={meals.filter(meal => new Date(meal.date_cooked).toLocaleDateString() === day.toLocaleDateString())}
      />
    </div>
  );
}
